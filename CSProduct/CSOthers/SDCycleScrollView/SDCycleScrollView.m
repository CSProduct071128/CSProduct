...
//            targetIndex = _totalItemsCount * 0.5;
            targetIndex = _imagePathsGroup.count * 1000;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:[imagePath stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]]] placeholderImage:self.placeholderImage];
        } else {
...
- (void)scrollToIndex:(NSUInteger)pageIndex
    ...
    if (pageIndex<self.imagePathsGroup.count) {
        BOOL oldAuto = self.autoScroll;
        self.autoScroll = NO;
        NSUInteger targetIndex = pageIndex;
        if (self.infiniteLoop) {
//            targetIndex = _totalItemsCount * 0.5+pageIndex;
            targetIndex = _imagePathsGroup.count * 1000 + pageIndex;
        }else{
            targetIndex = 0;
        }
        [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
        [self setAutoScroll:oldAuto];
    }else{
        NSLog(@"ivalide for pageIndex for sure ...");
    }
}
@end
